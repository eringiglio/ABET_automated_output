"""
Intended to pull the output of any given DB folder into a set of folders and files including: 1) a daily summary output for all mice run on a given day; 2) folders per mouse with a csv file for each mouse per day. This version is intended to pull only the most recent day and create its folders EMG
"""
import os
import pandas as pd
import datetime as dt

dbFolder = os.path.abspath(os.curdir + '/outputs/female9-12_DB27/') #get the folder path for where I am right now
...
i = max(scheduleDF.date.unique()) #this will always grab the most recent date in the dataset and break only that one out 
